...
🇫🇷 **[Français — vous êtes ici]** · 🇬🇧 **[English → README.en.md](README.en.md)**
...
![Fiches de chambre générées en PDF depuis un CSV — numéro de chambre, liste d'occupants et logo personnalisé](exemple.png)
...
| 🔍 | Détection du délimiteur | `csv.Sniffer` détecte `,` `;` `\t` — formats américain et européen pris en charge |
| 📐 | Mise en page dynamique | Bloc occupants ajusté selon le nombre de personnes (de 1 à 8+) |
| 🖼️ | Logo personnalisable | PNG redimensionné à 4×4 pouces, aligné à droite (fond transparent ou non) |
...
2. **Fichier → Enregistrer sous** → type : **CSV UTF-8 (délimité par des virgules) (\*.csv)**
   - Si l'option UTF-8 est absente, "CSV séparé par des points-virgules" fonctionne également.
